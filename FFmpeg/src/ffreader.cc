 ...
 :m_stream(nullptr),
  ...
  FFReader::FFReader()
   :m_filefmt_ctx{nullptr},
    m_srctype{SoureType::kSourceTypeUnknow},
    m_exit{false},
    m_cur_pos{0},
    m_seek_pos{0},
    m_eof{false},
...
bool FFReader::SetReadSourceType(SoureType src) {

       if(SoureType::kSourceTypeA==m_srctype && m_mediainfo.m_hasaudio) {
            m_srctype = SoureType::kSourceTypeA;
            m_curvalid_decode = &m_adecode;
        }else if(SoureType::kSourceTypeV==m_srctype && m_mediainfo.m_hasvideo) {
            m_srctype = SoureType::kSourceTypeV;
...
bool FFReader::GetSourceV(VideoSource *frame, uint64_t pos) {   
    if( SoureType::kSourceTypeV == m_srctype)
    {
...
bool FFReader::GetSourceA(AudioSource *frame, uint64_t pos) {
    if( SoureType::kSourceTypeA == m_srctype)
    {
...
int FFReader::SeekFile(uint64_t seek_pos) {
    ...
    int rec = av_seek_frame(m_filefmt_ctx, m_curvalid_decode->m_stream->index, m_seek_pts, AVSEEK_FLAG_BACKWARD);
    if (rec < 0)
    {
        return rec;
    }
